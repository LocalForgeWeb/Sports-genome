...
describe("About Me passkey management", () => {
  ...
  it("renders persistent existing-account passkey enrollment on a supported device", () => {
    ...
    expect(markup).toContain("Primary goal");
    expect(markup).toContain("Primary sport");
    expect(markup).toContain("Role / event / style");
    expect(markup).toContain("Training days / week");
    expect(markup).toContain("Available equipment");
    expect(markup).toContain("Automatic stack constraint");
  });
});
