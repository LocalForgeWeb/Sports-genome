...
describe("Body Lab architecture mechanics disclosure", () => {
  ...
  it("lets the figure fill the mobile canvas height without clipping", () => {
    // The library wrapped its SVG in a padded flex container and clamped itself
    // to 70vh inline, which needed !important to reach. Ours does neither.
    expect(styles).toContain(".atlas-body-chart svg{height:100%;width:auto;max-width:100%}");
