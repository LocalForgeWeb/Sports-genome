  ...
  return <section className="movement-atlas-improved">
    ...
    <div className="atlas-analysis-grid">
      ...
      <article className="atlas-focus-card">
        ...
        <details className="atlas-details"><summary>Sport → program reasoning <ChevronDown className="h-4 w-4" /></summary><dl><div><dt>Sport context</dt><dd>{reasoning.sport} · {reasoning.modifier}</dd></div><div><dt>Movement demand</dt><dd>{reasoning.biomechanics}</dd></div><div><dt>Priority qualities</dt><dd>{reasoning.physicalQualities.join(" · ")}</dd></div><div><dt>Target adaptations</dt><dd>{reasoning.adaptations.join(" · ")}</dd></div><div><dt>Training boundary</dt><dd>{reasoning.modality} {reasoning.exerciseBoundary}</dd></div></dl><p>{reasoning.evidenceBoundary}</p></details>
