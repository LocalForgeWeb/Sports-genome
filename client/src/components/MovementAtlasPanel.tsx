  ...
  return <section className="movement-atlas-improved">
    ...
    <div className="atlas-tool-row">
      <div className="atlas-tool-main">
        <div className="atlas-sport-select"><span>Sport lens</span><select value={sportId} onChange={(event) => { emitInteractionFeedback(); onSport(event.target.value); setModifierId(""); setShowAllActions(false); }} aria-label="Choose sport">{sports.map((sport) => <option key={sport.id} value={sport.id}>{sport.label}</option>)}</select></div>
      ...
      <div className="atlas-family-row"><button onClick={() => { emitInteractionFeedback(); onFamily("All"); }} aria-pressed={family === "All"}>All actions</button>{shownFamilies.map((item) => <button key={item} onClick={() => { emitInteractionFeedback(); onFamily(item); }} aria-pressed={family === item}>{item}</button>)}{families.length > 5 && <button className="atlas-more-filter" onClick={() => { emitInteractionFeedback(); setShowAllFamilies((value) => !value); }}>{showAllFamilies ? "Fewer filters" : `+${families.length - 5} filters`}</button>}</div>
    </div>

    <div className="atlas-analysis-grid">
      <div className="atlas-action-list" aria-label="Sport actions">
        {shownActions.length ? <>{shownActions.map((movement, index) => <button key={movement.id} onClick={() => { emitInteractionFeedback(); onMovement(movement); }} aria-pressed={movement.id === selectedMovement.id} className="atlas-action-item"><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{movement.label}</strong><small>{movement.family}</small></div><ArrowUpRight className="h-4 w-4" /></button>)}{visible.length > 7 && <button className="atlas-show-more" onClick={() => { emitInteractionFeedback(); setShowAllActions((value) => !value); }}>{showAllActions ? "Show fewer actions" : `Show all ${visible.length} actions`}<ChevronDown className={`h-4 w-4 ${showAllActions ? "rotate-180" : ""}`} /></button>}</> : <div className="atlas-empty"><strong>No actions match this filter.</strong><p>Clear the filter or try a broader muscle or action term.</p></div>}
      </div>
      <article className="atlas-focus-card">
        ...
        <button onClick={() => { emitInteractionFeedback(); onOpenBody(); }}>Trace in Body Lab <ArrowUpRight className="h-4 w-4" /></button>
      </article>
    </div>
  </section>;
}
