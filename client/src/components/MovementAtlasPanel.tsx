  ...
  return <section className="movement-atlas-improved">
    ...
    <div className="atlas-tool-row">
      <div className="atlas-tool-main">
        <div className="atlas-sport-select"><span>Sport lens</span><select value={sportId} onChange={(event) => { onSport(event.target.value); setShowAllActions(false); }} aria-label="Choose sport">{sports.map((sport) => <option key={sport.id} value={sport.id}>{sport.label}</option>)}</select></div>
