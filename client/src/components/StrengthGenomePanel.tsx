  ...
  return <section className="strength-genome-workspace space-y-5">
    ...
    <div className="strength-progress-grid grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
      <section className="strength-log-entry dark-panel p-5">
        ...
        <button type="button" disabled={!canSave || addObservation.isPending} onClick={submit} className="mt-5 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--sg-action)] px-4 text-[11px] font-bold uppercase tracking-[.12em] text-white transition active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"><Plus className="h-4 w-4" /> {addObservation.isPending ? "Saving" : "Save this lift"}</button>
      </section>
