    ...
    {latestRecord ? <>
      <article className="strength-region-record-card">
        ...
        {strengthTrend ? <article className="strength-reference-matched strength-reference-primary strength-rating-card" style={{ borderColor: changeStateCopy[strengthTrend.changeState].tone }}>
          ...
          <p>Estimated 1RM change across {strengthTrend.observationCount} logs since {strengthTrend.firstPoint.observedAt.toLocaleDateString()}.</p>
        </article> : <p className="strength-rating-empty">Log this lift again to see a progress rating here.</p>}
        {bodyMassRatio != null && <p className="strength-region-ratio-inline">{bodyMassRatio.toFixed(2)}× body mass on test day — supporting context only, not a rank.</p>}
        ...
        <details className="strength-region-boundary"><summary>{(powerliftingReference?.status === "matched" || piperReference?.status === "matched") ? "About this comparison" : "Why isn't there a population rank?"}</summary><p>{(powerliftingReference?.status === "matched" || piperReference?.status === "matched") ? "This is the primary result for this exact matched test only, not a general claim about this lift." : "A percentile appears only when the exercise, protocol, source population, and test-day body mass match a reviewed table. Your rating above is a within-athlete trend from your own logs, not a percentile, universal rank, or regional force score."}</p></details>
