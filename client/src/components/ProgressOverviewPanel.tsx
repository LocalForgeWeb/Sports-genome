...
export function ProgressOverviewPanel({ onOpenStrength, onOpenTraining }: { onOpenStrength: () => void; onOpenTraining: () => void }) {
  ...
  return <section className="progress-review space-y-4">
    <header className="progress-review-head">
      <div><p className="metric-label !text-[#9fc4eb]">Your training record</p><h1 className="mt-2 font-display text-5xl font-bold uppercase leading-[.82] text-white">Progress from<br /><em className="text-[#62a5ff]">real work.</em></h1><p className="mt-3 text-sm leading-5 text-[#c9dcef]">Every session you finished and every lift you logged.</p></div>
    ...
    <div className="grid gap-3 md:grid-cols-2">
      ...
      <section className="dark-panel progress-stat-card"><div><p className="metric-label !text-[#a9bed4]">Lifts logged</p><p className="mt-2 font-display text-6xl font-bold leading-none text-white">{observations.data?.length || 0}</p><p className="mt-2 text-xs leading-5 text-[#c4d3e2]">{latestObservation ? latestObservation.exerciseName : "No lifts logged yet."}</p></div><Dumbbell className="h-7 w-7 text-[#e4512e]" /><button type="button" onClick={onOpenStrength} className="progress-text-action">Strength Genome <Sparkles className="h-4 w-4" /></button></section>
    </div>
