...
export function ProgrammingGuidePanel({ workout, prescriptions, settings, goal }: { workout: Exercise[]; prescriptions: Record<number, string>; settings: Record<number, ExerciseSettings>; goal: TrainingGoal }) {
  ...
  return <section className="programming-guide-panel" aria-label="Goal-specific programming guide">
    <details className="programming-guide-disclosure">
      <summary>
        <div><p className="metric-label">Planning guide</p><h3>{goal}</h3><p>Adjustable prescription context.</p></div>
        <div className="programming-guide-summary-signal"><strong>{diagnostics.totalSets} work sets</strong><span>Open planning guide</span><BarChart3 className="h-5 w-5" /></div>
      </summary>
      <div className="programming-guide-details">
        ...
        <div className="programming-guide-stats"><div><span><Layers3 className="h-3.5 w-3.5" /> session working sets</span><strong>{diagnostics.totalSets}</strong><small>{inBand ? "Inside the current planning band" : `Reference band: ${target.sessionSetBand[0]}–${target.sessionSetBand[1]}`}</small></div><div><span><Clock3 className="h-3.5 w-3.5" /> work-set approach</span><strong>{target.workingSetCue}</strong><small>{target.repetitionCue}</small></div></div>
