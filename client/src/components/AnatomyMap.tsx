...
import { roleMapForLists, sourceValuesForRegion, viewsForRegion, type AnatomyRole } from "@/lib/anatomyRegions";
...
export function AnatomyMap({ primary, secondary, onSelect, muscleScores, roleDetails, roleMethodology, showInspector = true }: AnatomyMapProps) {
  ...
  const ranked = useMemo(() => {
    ...
    Object.keys(roles).forEach(key => {
      ...
      entries.push({ key, label: labels[key] || key, role: displayRole(key), roles: detail?.roles, confidence: detail?.confidence || "Low-confidence inference" });
    });
      ...
      <div className="atlas-pro-grid">
        ...
        <div className="atlas-pro-canvas">
          ...
          {showInspector && selectedKey && <div className="atlas-selected-strip">
            ...
            {selectedRole && <span className="atlas-selected-confidence">{selectedRoleDetail?.confidence || "Low-confidence inference"}</span>}
          ...
          {selectedKey ? (
            <>
              ...
              <div className="atlas-inspector-badges">
                ...
                {selectedRole && <i>{selectedRoleDetail?.confidence || "Low-confidence inference"}</i>}
