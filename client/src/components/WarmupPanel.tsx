...
export function WarmupPanel({ workout, goal }: { workout: Exercise[]; goal: TrainingGoal }) {
  ...
  return <section id="session-prep" className="warmup-panel" aria-label="Pre-training mobility recommendation"><div className="warmup-head"><div><p className="metric-label !text-[#93bde8]">Pre-training prep</p><h3>Move with intent</h3><p>{warmup.rationale}</p></div><div className="warmup-time"><Gauge className="h-4 w-4" /><strong>~{warmup.estimatedMinutes} min</strong><small>{warmup.drills.length} drills</small></div></div><div className="warmup-focus"><span>Stack signals</span><div>{warmup.focusTags.length ? warmup.focusTags.slice(0, 5).map((tag) => <b key={tag}>{tag.replace(/([A-Z])/g, " $1")}</b>) : <b>general prep</b>}</div></div>{/* The drills, their phases, their cues and their doses ran to six cards and
