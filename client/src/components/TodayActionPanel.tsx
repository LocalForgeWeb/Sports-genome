...
export function TodayActionPanel({ stagedExerciseCount, trainingDays, activeDayLabel, onOpenTraining, onOpenStrength, sexForReference, birthYear }: { stagedExerciseCount: number; trainingDays: number; activeDayLabel: string; onOpenTraining: () => void; onOpenStrength: () => void; sexForReference?: string; birthYear?: number }) {
  ...
  return <section className="today-action-panel">
    ...
    <div className="today-action-facts"><div><Dumbbell className="h-4 w-4" /><p><strong>{trainingDays} planned days</strong><span>your weekly rhythm</span></p></div><div><ClipboardCheck className="h-4 w-4" /><p><strong>{completedCount} completed sessions</strong><span>saved training history</span></p></div><button type="button" onClick={onOpenStrength}><Sparkles className="h-4 w-4" /><p><strong>{overview.data?.observationCount || 0} lifts logged</strong><span>{overview.data?.observationCount ? "continue your private record" : "log your first lift"}</span></p><ArrowUpRight className="h-4 w-4" /></button></div>
  </section>;
}
