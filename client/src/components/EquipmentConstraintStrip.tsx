...
export function EquipmentConstraintStrip({ profile, onOpenProfile }: { profile: AthleteEquipmentProfile; onOpenProfile: () => void }) {
  return <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border border-[#d9e2ec] bg-[#f4f8fc] px-4 py-3 text-xs text-[#38516a]">
    <div className="flex min-w-0 items-start gap-2"><Dumbbell className="mt-0.5 h-4 w-4 shrink-0 text-[#e4512e]" /><p><strong className="font-bold text-[#173d69]">Automatic stack filter:</strong> {equipmentProfileSummary(profile)} This applies to recommendations, smart drafts, generated weeks, and active training days; the catalog remains available for manual additions.</p></div>
