...
export function ProgressionReviewSummary({ exercises, history, onApprove, onApproveSegment }: { exercises: ProgressionExercise[]; history: LoggedPerformanceSet[]; onApprove?: (recommendation: ExerciseProgressionRecommendation) => void; onApproveSegment?: (signal: MuscleSegmentSignal) => void }) {
  ...
  return <section className="progression-review-panel" aria-label="Progression review">
    ...
    {segments.length > 0 && <div className="progression-segments"><div><p className="metric-label">Segment performance signals</p><p>Exercise-context proxy; compare within your own logs, not against a universal muscle-strength ranking.</p></div>{segments.map((signal) => <article key={signal.muscle}><span className={`progression-status progression-status-${signal.status}`}>{signal.status}</span><strong>{signal.muscle.replace(/_/g, " ")}</strong><p>{signal.rationale}</p>{signal.status === "review" && onApproveSegment && <button type="button" className="mt-2 inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-[.1em] text-[#2d6cdf]" onClick={() => onApproveSegment(signal)}><Check className="h-3.5 w-3.5" /> Mark for next plan review</button>}</article>)}</div>}
...
export function ProgressionReviewPanel({ workout, prescriptions, settings, bodyWeight, weightUnit, onApprove, onApproveSegment }: { workout: Exercise[]; prescriptions: Record<number, string>; settings: Record<number, ExerciseSettings>; bodyWeight?: number; weightUnit?: "lb" | "kg"; onApprove?: (recommendation: ExerciseProgressionRecommendation) => void; onApproveSegment?: (signal: MuscleSegmentSignal) => void }) {
  ...
  return <ProgressionReviewSummary exercises={exercises} history={history} onApprove={onApprove} onApproveSegment={onApproveSegment} />;
}
