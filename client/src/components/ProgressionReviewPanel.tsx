...
export function ProgressionReviewSummary({ exercises, history, catalog = [], availableEquipment = [], onApprove, onApproveSegment, onAddSuggestion }: { exercises: ProgressionExercise[]; history: LoggedPerformanceSet[]; catalog?: Exercise[]; availableEquipment?: string[]; onApprove?: (recommendation: ExerciseProgressionRecommendation) => void; onApproveSegment?: (signal: MuscleSegmentSignal) => void; onAddSuggestion?: (suggestion: SegmentPrioritySuggestion) => void }) {
  ...
  return <section className="progression-review-panel" aria-label="Progression review">
    ...
    <div className="progression-recommendations">{recommendations.map((recommendation) => {
      ...
      return <article key={recommendation.exerciseId} className="progression-recommendation"><div><p className="metric-label">{recommendation.exerciseName}</p><p className={`mt-1 flex items-center gap-1 text-xs font-bold ${presentation.tone}`}><Icon className="h-3.5 w-3.5" />{presentation.label} <span className="font-normal text-[#657b92]">· {recommendation.confidence} confidence</span></p></div><p>{recommendation.rationale}</p>{recommendation.relativePerformance !== undefined && <small className="mt-2 block text-[#657b92]">Relative to your bodyweight: {recommendation.relativePerformance.toFixed(2)}. Only meaningful next to your own past logs for this exercise.</small>}{canApprove && onApprove && <button type="button" className="mt-3 inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-[.1em] text-[#2d6cdf]" onClick={() => onApprove(recommendation)}><Check className="h-3.5 w-3.5" /> Apply as next-session note</button>}</article>;
    })}</div>
    {segments.length > 0 && <div className="progression-segments"><div><p className="metric-label">How each muscle group is trending</p><p>Read from your own logs for these exercises. It is not a ranking against other people.</p></div>{segments.map((signal) => {
      ...
      return <article key={signal.muscle}><span className={`progression-status progression-status-${signal.status}`}>{signalStatusCopy[signal.status]}</span><strong>{signal.muscle.replace(/_/g, " ")}</strong>{signal.progressIndex !== undefined && <p className="mt-1 text-[10px] font-bold uppercase tracking-[.08em] text-[#47617a]">Your trend score: {signal.progressIndex}/100</p>}<p>{signal.rationale}</p>{signal.status === "review" && onApproveSegment && <button type="button" className="mt-2 inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-[.1em] text-[#2d6cdf]" onClick={() => onApproveSegment(signal)}><Check className="h-3.5 w-3.5" /> Mark for next plan review</button>}{signal.status === "review" && suggestions.length > 0 && <div className="mt-3 border-t border-[#d8e4ef] pt-3"><p className="text-[10px] font-bold uppercase tracking-[.09em] text-[#657b92]">Optional next-plan additions</p>{suggestions.map((suggestion) => <button key={suggestion.exerciseId} type="button" className="mt-2 flex w-full items-center justify-between gap-2 text-left text-xs font-bold text-[#2d6cdf]" onClick={() => onAddSuggestion?.(suggestion)}><span>Add {suggestion.exerciseName}</span><span className="font-normal text-[#657b92]">{suggestion.equipment}</span></button>)}<small className="mt-2 block text-[#657b92]">{suggestions[0].boundary}</small></div>}<small className="mt-2 block text-[#657b92]">{signal.boundary}</small></article>;
    })}</div>}
