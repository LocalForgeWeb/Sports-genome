...
describe("Training Day exercise prescription row", () => {
  it("keeps exercise controls distinct while allowing direct set and repetition editing plus duplication", () => {
    ...
    expect(markup).toContain(`${exercise.name} sets`);
    expect(markup).toContain(`${exercise.name} repetitions or target`);
    expect(markup).toContain(`Duplicate ${exercise.name} prescription`);
    expect(markup).toContain("Reps / target");
