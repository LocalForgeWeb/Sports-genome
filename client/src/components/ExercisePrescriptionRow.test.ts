...
describe("how many sets", () => {
  it("nudges the count up and down rather than asking anyone to type it", () => {
    ...
    expect(onPrescription).toHaveBeenLastCalledWith("2 × 8");
  });

  it("will not go below one set", () => {
    renderRow("1 × 8");
    expect(screen.getByLabelText(`One fewer set of ${exercise.name}`).hasAttribute("disabled")).toBe(true);
  });
...
describe("what the sets ask for", () => {
   ...
   */
  it("opens a field per set on request without inventing an edit", () => {
    const { onPrescription } = renderRow("3 × 8");
  ...
  it("does not offer to vary a single set, because there is nothing to vary against", () => {
    renderRow("1 × 8");
    expect(screen.getByRole("button", { name: "Vary by set" }).hasAttribute("disabled")).toBe(true);
  });
});
