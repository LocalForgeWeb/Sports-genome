...
describe("Rate Stack split-target index", () => {
  ...
  it("keeps Training Day prescription controls on accessible high-contrast navy and blue tokens", () => {
    expect(globalCss).toContain(".destination-train .day-programming-panel .metric-label");
    expect(globalCss).toContain("color: #edf7ff");
    expect(globalCss).toContain(".destination-train .day-programming-panel select");
    expect(plannerCss).toContain("background: #0b2240");
    expect(plannerCss).toContain("color: #f7fbff");
    expect(plannerCss).toContain("color: #c3d7ea");
