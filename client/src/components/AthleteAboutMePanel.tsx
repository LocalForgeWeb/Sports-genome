...
export function AthleteAboutMePanel({ baseline, goal, trainingDays, sportId, sports, onBaseline, onGoal, onDays, onSport }: {
  ...
  return <section className="about-me-panel">
    <div className="about-me-head"><div><p className="metric-label">Athlete profile / editable</p><h1>About <em>me.</em></h1><p>Planning context only — editable inputs that guide stack availability, not health or ability ratings.</p></div><UserRound className="h-7 w-7 text-[#e4512e]" /></div>
    <div className="about-me-grid">
      <section className="about-me-card"><div className="about-me-card-head"><UserRound className="h-4 w-4" /><span>Identity & experience</span></div><label><span>Preferred name</span><input value={baseline.preferredName || ""} placeholder="Add a name" onChange={(event) => onBaseline({ ...baseline, preferredName: event.target.value || undefined })} /></label><label><span>Training experience</span><select value={baseline.experience} onChange={(event) => { emitInteractionFeedback(); onBaseline({ ...baseline, experience: event.target.value as AthleteExperience }); }}>{experiences.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>Bodyweight (optional)</span><div className="about-me-inline"><input inputMode="decimal" value={baseline.bodyWeight || ""} placeholder="Not added" onChange={(event) => { const bodyWeight = Number(event.target.value); onBaseline({ ...baseline, bodyWeight: Number.isFinite(bodyWeight) && bodyWeight > 0 ? bodyWeight : undefined }); }} /><select value={baseline.weightUnit} onChange={(event) => { emitInteractionFeedback(); onBaseline({ ...baseline, weightUnit: event.target.value as WeightUnit }); }}><option value="lb">lb</option><option value="kg">kg</option></select></div></label><label><span>Sex for population-reference comparisons (optional)</span><select value={baseline.sexForReference || ""} onChange={(event) => { emitInteractionFeedback(); onBaseline({ ...baseline, sexForReference: (event.target.value || undefined) as SexForReference | undefined }); }}><option value="">Not set</option><option value="female">Female</option><option value="male">Male</option><option value="intersex">Intersex</option><option value="unspecified">Prefer not to say</option></select></label><label><span>Birth year (optional)</span><input inputMode="numeric" value={baseline.birthYear || ""} placeholder="e.g. 1998" onChange={(event) => { const birthYear = Number(event.target.value.replace(/[^0-9]/g, "").slice(0, 4)); const currentYear = new Date().getFullYear(); onBaseline({ ...baseline, birthYear: Number.isFinite(birthYear) && birthYear > currentYear - 100 && birthYear <= currentYear ? birthYear : undefined }); }} /></label></section>
