...
export function EmailAuthScreen({ onAuthenticated, loading }: { onAuthenticated: () => void; loading: boolean }) {
  ...
  if (offerPasskey) return <div className="email-auth-shell"><div className="email-auth-grid" /><main className="email-auth-card email-auth-success"><div className="email-auth-icon"><Fingerprint /></div><p className="pulse-kicker">Account created</p><h1>Use <em>Face ID</em><br />next time.</h1><p>Save a passkey on this device for quick, passwordless Sports Genome sign-in. Your device may use Face ID, Touch ID, or its secure screen lock.</p><button onClick={enrollPasskey} disabled={pending}><Fingerprint className="h-4 w-4" /> Enable Face ID / passkey</button><button className="email-auth-secondary" onClick={refreshAndContinue} disabled={pending}>Continue with email</button></main></div>;

  return <div className="email-auth-shell"><div className="email-auth-grid" /><header className="email-auth-brand"><img src={sportsGenomeAssets.officialLogo} alt="Sports Genome logo" /><div><strong>Sports Genome</strong><span>Private athlete workspace</span></div></header><main className="email-auth-card"><p className="pulse-kicker">Athlete account / secure access</p><h1>{mode === "signIn" ? <>Train on your<br /><em>own account.</em></> : <>Start your<br /><em>training record.</em></>}</h1><p>Use your Sports Genome email account to keep plans, equipment settings, favorites, and workout history private to you.</p><form onSubmit={submit}><label><span>Email</span><div><Mail className="h-4 w-4" /><input autoComplete="email" type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" required /></div></label><label><span>Password</span><div><KeyRound className="h-4 w-4" /><input autoComplete={mode === "signIn" ? "current-password" : "new-password"} type="password" value={password} onChange={event => setPassword(event.target.value)} placeholder={mode === "register" ? "At least 12 characters" : "Your password"} required /></div></label>{mode === "register" && <label><span>Confirm password</span><div><KeyRound className="h-4 w-4" /><input autoComplete="new-password" type="password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} placeholder="Repeat password" required /></div></label>}<button type="submit" disabled={pending}>{mode === "signIn" ? "Sign in with email" : "Create email account"} <Sparkles className="h-4 w-4" /></button></form>{passkeySupported && <button className="email-auth-passkey" onClick={signInWithPasskey} disabled={pending}><Fingerprint className="h-4 w-4" /> Sign in with Face ID / passkey</button>}<button className="email-auth-switch" onClick={() => { setMode(mode === "signIn" ? "register" : "signIn"); setPassword(""); setConfirmPassword(""); }} disabled={pending}>{mode === "signIn" ? "Need an account? Create one" : "Already have an account? Sign in"}</button><small>Your password is stored as a secure one-way hash. Face ID stays on your device; Sports Genome receives only your passkey verification.</small></main></div>;
}
