...
type Workspace = "command" | "profile" | "recommended" | "custom" | "day-plan" | "body" | "movement" | "catalog" | "genome";
...
export default function Home() {
  ...
  return <div className="apex-shell">
    ...
    <aside className={`apex-rail ${railOpen ? "rail-open" : ""}`}>
      ...
      <div className="rail-brand"><img src="/manus-storage/sports-genome-icon-192_ae889a25.png" alt="Sports Genome icon" className="h-10 w-10 shrink-0 object-contain" /><div><p className="font-display text-[22px] font-bold leading-[.72] tracking-wide text-white">SPORTS<br />GENOME</p><p className="mt-1 text-[9px] font-bold uppercase tracking-[.18em] text-[#9fc5ff]">Athlete performance system</p></div><button onClick={() => setRailOpen(false)} className="ml-auto text-[#c8d8e7] lg:hidden" aria-label="Close navigation"><X className="h-5 w-5" /></button></div>
      <div className="rail-athlete"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#e4512e] font-display text-xl font-bold text-white">{(user?.name || "A").slice(0, 1).toUpperCase()}</div><div><p className="text-xs font-bold text-white">{user?.name || "Athlete"}</p><p className="mt-0.5 text-[10px] text-[#b8cbe0]">Secure athlete workspace</p></div><UsersRound className="ml-auto h-4 w-4 text-[#b8cbe0]" /></div>
      ...
      <div className="rail-bottom"><div className="rail-data-card"><p className="metric-label">Research engine</p><div className="mt-2 flex items-end justify-between"><p className="font-display text-4xl font-bold text-white">{sportProfiles.length}</p><span className="mb-1 text-[10px] font-bold text-[#f7cf6c]">SPORTS</span></div><p className="mt-1 text-[11px] leading-4 text-[#b8cbe0]">{sportMovementProfiles.length} movement profiles linked to {exercises.length} exercises.</p></div><a href="https://localforgeweb.com" target="_blank" rel="noreferrer" className="mt-4 block text-[10px] text-[#9ab1ca]">built by Gabe Naim-LocalForgeWeb</a></div>
    </aside>
    ...
    <div className="apex-main">
      ...
      <Suspense fallback={<main className="apex-content"><div className="light-panel p-6 text-sm text-[#58728e]">Loading Exercise Genome analysis…</div></main>}><main className={`apex-content ${workspace === "catalog" ? "catalog-mode-active" : ""}`}>
        <EquipmentConstraintStrip profile={athleteBaseline.equipment} onOpenProfile={() => navigateWorkspace("profile")} />
