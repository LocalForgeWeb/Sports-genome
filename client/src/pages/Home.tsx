...
export default function Home() {
  ...
  const sessionRecommendations = useMemo(() => getSportSession(activeSportId, goal, gymTimeBudget.recommendationLimit, athleteBaseline.equipment), [activeSportId, goal, gymTimeBudget.recommendationLimit, athleteBaseline.equipment]);
  ...
  const draftedLoadout = useMemo(() => {
    const sportSeed = getSportSession(activeSportId, goal, Math.max(8, gymTimeBudget.recommendationLimit + 3), athleteBaseline.equipment).map((item) => item.exercise);
  ...
  }, [activeSportId, goal, activeSplitDay, activeLoadout, gymTimeBudget.recommendationLimit, athleteBaseline.equipment]);
  ...
  const generateWeek = () => {
    ...
    const sportSeed = getSportSession(activeSportId, goal, Math.max(10, gymTimeBudget.recommendationLimit + 4), athleteBaseline.equipment).map((item) => item.exercise);
  ...
  const completeOnboarding = ({ goal: selectedGoal, trainingDays: selectedDays, sportId: selectedSportId, stackMode, baseline }: AthleteQuizSelection) => {
    ...
    if (stackMode === "suggested") {
      setCustomWorkout(getSportSession(selectedSportId, selectedGoal, Math.min(6, selectedDays + 2), baseline.equipment).map((result) => result.exercise));
