...
import { Activity, ArrowUpRight, BarChart3, BookOpen, BrainCircuit, ChevronRight, ClipboardPaste, Dna, Dumbbell, Layers3, Menu, Move3d, Plus, Search, Settings2, ShieldCheck, SlidersHorizontal, Sparkles, Target, Trophy, UsersRound, X, Zap } from "lucide-react";
...
const navItems: { id: Workspace; label: string; icon: typeof Target; detail: string }[] = [
  ...
  { id: "day-plan", label: "Training Days", icon: Layers3, detail: "design each saved day" },
...
function RecommendationRow({ result, index, onAdd, onInspect }: { result: MovementRecommendation; index: number; onAdd: () => void; onInspect: () => void }) {
  return <div className="recommendation-row"><span className="recommendation-index">{String(index + 1).padStart(2, "0")}</span><button onClick={onInspect} className="recommendation-copy"><p>{result.exercise.name}</p><small>{result.rationale}</small></button><div className="recommendation-demand"><p>Shared demand</p><small>{result.matchedSignals.join(" · ") || "support"}</small></div><GradeStamp grade={result.grade} compact /><button onClick={onAdd} className="recommendation-add" aria-label={`Add ${result.exercise.name} to custom workout`}><Plus className="h-4 w-4" /></button></div>;
}
