  ...
  return <div className="apex-shell">
    ...
    <div className="mobile-workspace-dock lg:hidden" aria-label="Current workspace actions">
      <div className="mobile-workspace-actions">
        ...
        {workspace === "profile" && <><span><strong>About me</strong><small>Sport, goals, schedule, and equipment</small></span><button type="button" onClick={() => navigateWorkspace("recommended")}>Recommendations</button><button type="button" onClick={() => navigateWorkspace("day-plan")}>Review plan</button></>}
        {workspace === "movement" && <><span><strong>Movement Atlas</strong><small>Selected sport action and body requirements</small></span><button type="button" onClick={() => { setActiveMuscle(getMovementMuscles(selectedMovement)[0] || "abs"); navigateWorkspace("body"); }}>Open Body Lab</button><button type="button" onClick={() => navigateWorkspace("recommended")}>View matches</button></>}
        {workspace === "body" && <><span><strong>Body Lab</strong><small>Qualitative role map for the selected action</small></span><button type="button" onClick={() => { setCatalogFilters({ ...defaultCatalogFilters, muscle: activeMuscle }); navigateWorkspace("catalog"); }}>Find exercises</button><button type="button" onClick={() => navigateWorkspace("movement")}>Change action</button></>}
