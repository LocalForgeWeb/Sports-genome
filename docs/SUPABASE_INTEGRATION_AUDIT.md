...
The athlete-facing repository inventory intentionally excludes `staging_studies`. This protected research-intake table returned an access error through its direct REST path and is not appropriate for athlete-facing status context. Removing that count restored the clean live Strength Genome source-library summary—2,172 strength-norm records, 1,094 performance-norm records, and 56 standardized tests—without exposing staging records or weakening their access boundary.
